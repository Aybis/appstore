...
4. **No Supabase project exists for this application.** The two on the account (`Maya-Buddy`, `CMS`) are unrelated; only read-only catalog queries were run against `CMS`. A project must be created before any of §4's Supabase column can be filled in.
